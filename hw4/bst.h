...
protected:
    ...
    bool isBalancedHelper(Node<Key, Value>* curr, int& height) const;
...
{
    // TODO
    current_ = successor(current_); //return a pointer to its successor
    return *this;
}

template<class Key, class Value>
Node<Key, Value>* BinarySearchTree<Key, Value>::iterator::successor(Node<Key, Value>* current)
{
    Node<Key, Value>* right = current->getRight();
    //Node<Key, Value>* parent = current->getParent();
    Node<Key, Value>* successor;
    
    if (right != NULL) //has a right child, find the leftmost child of the right child
    {
        successor = right;
    ...
    else //has no right child, "walk up the ancestor chain until you traverse the first left child pointer (find the first node who is a left child of his parent...that parent is the successor"
    {
        //parent = current;
...
//void insertHelper(const std::pair<const Key, Value>& keyValuePair)
//{
//    if (root_ = NULL) return; //root_ would not be NULL when called
//    Node<Key,Value>* n = new Node(keyValuePair.first, keyValuePair.second, NULL);
...
//        n->setParent(temp);
//    }
//    if (keyValuePair.first > temp->getKey() && temp->getRight() == NULL)
//    {
//        temp->setRight(n);
//        n->setParent(temp);
//    }
...
void BinarySearchTree<Key, Value>::insert(const std::pair<const Key, Value> &keyValuePair)
{
    ...
    if(it != this->end())  //already exists, update
    {
...
void BinarySearchTree<Key, Value>::remove(const Key& key)
{
    ...
    if (it == this->end()) //doesn't exists, do nothing
    {
        return;
    }
    
    Node<Key,Value>* n = it.current_;
...
BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current)
{
    ...
    if (left != NULL) //has a left child, find the right most child of the left child
    {
        predecessor = left;
...
BinarySearchTree<Key, Value>::getSmallestNode() const
{
    ...
    while (smallestNode != NULL && smallestNode->getLeft() != NULL)
    {
...
template<typename Key, typename Value>
bool BinarySearchTree<Key, Value>::isBalancedHelper(Node<Key, Value>* curr, int& height) const
{
    int leftHeight = 0;
    int rightHeight = 0; //can't = 0 & have to initialze. how?
    ...
    isLeftBalanced = isBalancedHelper(curr->getLeft(), leftHeight);
    isRightBalanced = isBalancedHelper(curr->getRight(), rightHeight);
...
bool BinarySearchTree<Key, Value>::isBalanced() const
{
    // TODO
    int height = 0;
    return isBalancedHelper(root_, height);
}
